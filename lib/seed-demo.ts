...
// 示範內容(2 系列、4 課程、12 單元、mock 範例影片)。
// 全部 upsert,可重複執行不會重複建立。
// 供 prisma/seed.ts(指令列)與後台「一鍵載入示範資料」(server action)共用。

async function ensureVideo(prisma: PrismaClient, key: string) {
...
export async function seedDemoContent(prisma: PrismaClient): Promise<string> {
  ...
  for (const c of courses) {
    ...
    if (existing === 0) {
      for (let i = 0; i < c.lessons.length; i++) {
        const video = await ensureVideo(prisma, `sample-${c.slug}-${i + 1}`);
  ...
  return `示範資料就緒:2 系列、${courses.length} 課程(本次新增 ${createdLessons} 個單元)`;
}
