...
**關於免費額度:** Google AI Studio 免費方案每顆模型每天有呼叫次數上限(視模型而定,少的只有 20 次/天)。
用完當天會顯示「今天的占卜次數已達上限,請明天再來」,隔天自動恢復,**客人被扣掉的占卜次數會自動退還**。
預設用的 `gemini-flash-lite-latest` 額度較寬鬆且回應快;如果之後客人變多不夠用,
到 [aistudio.google.com](https://aistudio.google.com) 開啟計費(有免費額度、超過才按量計價)即可解除上限。
