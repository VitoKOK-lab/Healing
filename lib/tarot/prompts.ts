...
// 「一件事」的時間錨點輪替:寫死「明天就能做」會讓模型每篇都用
// 「明天起床,你可以⋯」開場(店主實測回報的單一化問題)。
// 錨點跟著角度輪,重生成時一起換。
...
不得出現「納瓦爾」。
第四段(一件事):只有一個動作,具體、做完看得出有沒有做。一到兩句,不得並列多個動作。
這一次的時間錨點用「${TIME_ANCHORS[opts.angleIndex % TIME_ANCHORS.length]}」或牌面自然長出的時機。
絕對禁止以「明天起床」「明天一早」「明天開始」開場——這是上一版被客人抓到的罐頭句型。
結尾句式每次都要不同:可以是行動指令、可以是一個約定、可以是一句斷言收尾,不可與定調段句式重複。
